...
def unevenLightCompensate(gray, blockSize):
    ...
    blockImage2 = cv2.resize(b, (gray.shape[1], gray.shape[0]), interpolation=cv2.INTER_CUBIC) #0.4的时候3个指标都提升了0.001
...
if __name__ == '__main__':
    ...
    for k in range(11):
        ...
        for file in file_name2:
            ...
            ret2,thresh2 = cv2.threshold(img, 6, 255, cv2.THRESH_BINARY_INV)
            img_and = cv2.bitwise_and(img, thresh1)
            img_not = cv2.bitwise_and(img,thresh2)
            # cv2.imshow("not",img_not)
            # cv2.waitKey(0)

            dst = unevenLightCompensate(img_and, blockSize)
            dst = dst + img_not
            # cv2.imshow("show",dst)
            # cv2.waitKey(0)
            ...
            if not os.path.exists("image_{}/Lane01/".format(name)+cycle_name):
                os.makedirs("image_{}/Lane01/".format(name)+cycle_name)
