...
def main():
    ...
    for k in range(11):
        if k < 10:
            ...
            str_k = str(k)
        file_name1 = r"..\\image_4\Lane01\Cyc0{}".format(str_k)
        ...
        for file in file_name2:
            ...
            for i in range(8):
                ...
                if not os.path.exists("../processed_roi/s{}/Lane01/".format(i)+cycle_name):
                    os.makedirs("../processed_roi/s{}/Lane01/".format(i)+cycle_name)

                img_temp1 = img_and[:, i * step:(i + 1) * step].copy()
                img_and[:, i * step:(i + 1) * step] = 1
                img_and[:, 3 * 512:4 * 512] = img_temp1

                save_name = "../processed_roi/s{}/Lane01/".format(i) + cycle_name + "/" + image_name
