...
def main():
    ...
    train_img_files = [f for f in all_img_files if "08h_" not in f and "44h" not in f and "pure" in f]
    ...
    val_img_files = [f for f in all_img_files if "44h"  in f and "pure" in f]
    ...
    train_loader = data.DataLoader(train_dataset, batch_size=batchsize, num_workers=1, shuffle=True, drop_last=True)
    ...
    if continueTrain == True:
        best_model = torch.load("savedir_pth_deep_pure/acc99.2537_epoch22_loss0.0290.pth.tar")['state_dict']
