...
import numpy as np
from model_dir.nested_model import NestedUNet
import random
...
def main():
    ...
    train_img_files = [f for f in all_img_files if "44h" not in f and "pure" in f]
    ...
    for path,path_label in zip(train_img_files,train_label_files):
        data_list_, data_label_list_ = npy_read_as_list(path, path_label,height=height)
    ...
    val_data_list,val_label_list = npy_read_as_list(val_img_files[0],val_label_files[0],label=True,height=height)
    ...
    if continueTrain == True:
        best_model = torch.load("savedir_nested_pth/acc99.358.pth.tar")['state_dict']
    ...
    for epoch in range(10000):
        ...
        for inputs, labels in train_loader:
            idx += 1
            model.train()
            #随机取前96个cycle或者后96个cycle
            random_numeber = random.random()
            # if random_numeber>0.5:
            #     inputs = inputs[:,:,:,:96]
            #     labels = labels[:,:,:,:96]
            # else:
            #     inputs = inputs[:,:,:,-96:]
