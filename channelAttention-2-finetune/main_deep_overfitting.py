...
import numpy as np
from mymodel import DNA_Sequencer_Atten_Deep as DNA_Sequencer_Atten
...
def main():
    ...
    """
    continueTrain = True
    save_dir = 'savedir_pth_deep_pure/'
    os.makedirs(save_dir,exist_ok=save_dir)
    best_acc = 0
    batchsize = 4
    height = 1000
    dir_name = r"E:\data\liangdujuzhen\img"

    all_img_files = glob.glob(r"E:\data\liangdujuzhen\img\*.npy")
    train_img_files = [f for f in all_img_files if "_0" not in f and "44h" not in f and  "pure" in f]
    ...
    train_loader = data.DataLoader(train_dataset, batch_size=batchsize, num_workers=2, shuffle=True, drop_last=True)
    ...
    if continueTrain == True:
        best_model = torch.load("savedir_pth_deep_pure/acc99.2266_epoch455_loss0.0379.pth.tar")['state_dict']
    ...
    for epoch in range(10000):
        ...
        print('Epoch [%d] loss = %.4f, total acc: %.4f %%' % (epoch + 1, loss_show.avg, accurate_show.avg))
        # loss_show.reset()
        # accurate_show.reset()
        # with torch.no_grad():
        ...
        }, save_dir=save_dir,filename='acc{:.4f}_epoch{}_loss{:.4f}.pth.tar'.format(accurate_show.avg, epoch, loss_show.avg))
        loss_show.reset()
        accurate_show.reset()
