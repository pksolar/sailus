...
def main():
    ...
    # 切块以后，还剩余的数据也要补足上去，但是会太多，因此最后一步要剪掉。

    val_data_list.append(val_data[-height:]) #num_pic2*height
    val_label_list.append(val_label[-height:])
    duoyu = height - (num2-num_pic2*height) #最后一块多算的
    ...
    best_model = torch.load("savedir_nested_pth/acc99.943.pth.tar")['state_dict']
    ...
    with torch.no_grad():
        for inputs, labels in val_loader: #
            ...
            print('Iter{} of {} '.format(idx, len(val_loader)))
            print('acc:%.3f %%' % ( accurate))

            pred_np = pred.cpu().numpy().astype(int).squeeze() # 1,1000,100
            str_acgt = ""
            for i in range(pred_np.shape[0]): # 第i条reads
                #如果是最后一块，则在前duoyu个reads不保存。
    ...
    # 将坐标保存到txt文件
    fastQ.writeFq(save_path + 'Lane01_fastq.fq', predict_list, 'R001C001')
    end = time.time()
    print("time:", end - s)
    autoMap(save_path)
