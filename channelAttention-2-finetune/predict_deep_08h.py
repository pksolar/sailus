...
def main():
    ...
    val_path = dir_name + "44h_R001C001_withNoMap.npy"#_update
    fov = "44h_R001C001"
    ...
    val_loader = data.DataLoader(val_dataset, batch_size=batchsize, num_workers=1, shuffle=False, drop_last=False)

    model = DNA_Sequencer_Atten().to(device)
    # 定义损失函数和优化器
    #best_model = torch.load("savedir_pth/acc98.731.pth.tar")['state_dict']
    best_model = torch.load("savedir_pth_deep_pure/acc99.7469_epoch3_loss0.0088.pth.tar")['state_dict']
