...
from datasets import Dataset_3cycle, Dataset_3cycle_test
from utils import *
import sys
from model.model_NestedUnet import NestedUNet
from val_mapping_total import automap_delelte
import  threading
...
def main():
    ...
    train_loader = data.DataLoader(train_dataset, batch_size=BATCHSIZE, num_workers=6, shuffle=True, pin_memory=True)
    # val_dir = glob.glob(r"E:\code\python_PK\callbase\datasets\{}\Res\Lane01\deepLearnData\*\intensity_norm\*_A.npy".format(
    #     dataset_name_val))[:5]
    # val_dataset = Dataset_3cycle_test(val_files)
