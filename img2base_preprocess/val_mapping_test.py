...
import numpy as np
"""
use the file in pth to do mapping ,pick the model with highest mapping rate.
"""

import os
import torch
from datasets import Dataset_3cycle_val_mapping
...
def main(dir_filename):
    ...
    BATCHSIZE = 1
    test_root_dir = r"E:\data\deepData\val_mapping\img"
    ...
    test_dataset = Dataset_3cycle_val_mapping(test_files)
    ...
    with torch.no_grad():
        ...
        for inputs, labels, msk,filename in test_loader:  #注意此处的msk 是否包含mapping上的
            ...
            msk = torch.squeeze(msk).numpy().flatten().astype(int)  # msk b,1,b,w
            ...
            # sys.stdout.write("\r" +rf"accuray:{accurate:.4f},total accuracy:{accurate_show.avg:.5f}" )
            # sys.stdout.flush()
            ...
            label_total = label_total + 1  # 从1开始。

            # for i, label_ele in enumerate(label_total):
            #     if cycle == 1:  # 说明是第一个cycle，创建包含几个reads的列表 ，
            #         labelacgt.append(dictacgt[label_ele])
            #     else:  # 第二个cycle以后依次写入：
            #         labelacgt[i] = labelacgt[i] + dictacgt[label_ele]
            print("cycle:", cycle)
            cycle += 1
            if cycle >40 :
                break
...
if __name__ == '__main__':
    ...
    filename = r"E:\code\python_PK\img2base_preprocess\pth_备用\acc99.3356_epoch0.pth.tar"
    main(filename)
