...
def save_image_tensor2cv2(input_tensor: torch.Tensor, cycname,imgname):
    ...
    cycdirname = rf"E:\code\python_PK\VoxelMorph-torch-master\reg\deformed_grid_img\{cycname}"
...
def train():
    ...
    #flow load:
    flow0 =torch.Tensor(np.load(r"E:\code\python_PK\VoxelMorph-torch-master\Result\temp\0_round_flow.npy")[np.newaxis,:]).to(device).float()
    flow1 = torch.Tensor(np.load(r"E:\code\python_PK\VoxelMorph-torch-master\Result\temp\1_round_flow.npy")[np.newaxis,:]).to(device).float()
    flow2 = torch.Tensor(np.load(r"E:\code\python_PK\VoxelMorph-torch-master\Result\temp\2_round_flow.npy")[np.newaxis,:]).to(device).float()
    with torch.no_grad():

        for i,path  in enumerate(imgs_path):
            #imgA =torch.Tensor(normalize(cv2.imread(path,0))[np.newaxis,np.newaxis,:]).to(device).float()
            imgC =torch.Tensor(cv2.imread(path.replace("_A","_C"),0)[np.newaxis,np.newaxis,:]).to(device).float()
            imgG =torch.Tensor(cv2.imread(path.replace("_A","_G"),0)[np.newaxis,np.newaxis,:]).to(device).float()
            imgT =torch.Tensor(cv2.imread(path.replace("_A", "_T"), 0)[np.newaxis,np.newaxis,:]).to(device).float()
            ...
            output0 = STN(imgC, flow0)
            output1 = STN(imgG, flow1)
            output2 = STN(imgT, flow2)
            ...
            save_image_tensor2cv2(output0, cycname,"R001C001_C.tif")
            save_image_tensor2cv2(output1, cycname, "R001C001_G.tif")
            save_image_tensor2cv2(output2, cycname, "R001C001_T.tif")
