...
def train():
    ...
    # 读入fixed图像
    f_img = normalize(cv2.imread("images/testinv/A_0.tif",0)[np.newaxis,np.newaxis,:])
    f_img = torch.Tensor(f_img)
    input_fixed = f_img.to(device).float()

    C_img = normalize(cv2.imread("images/testinv/C_0.tif",0)[np.newaxis,np.newaxis,:])
    G_img = normalize(cv2.imread("images/testinv/C_0.tif",0)[np.newaxis,np.newaxis,:])
    T_img = normalize(cv2.imread("images/testinv/C_0.tif", 0)[np.newaxis,np.newaxis, :])
    imglist = [C_img]
    # 创建配准网络（UNet）和STN，
    #pk:encoder层是一样的。
    ...
    with torch.no_grad():
        for i,img  in enumerate(imglist):
            ...
            # Save images
            m_name = "images/testinv/"+str(i) + "-50_m.tif"
            m2f_name = "images/testinv/"+str(i) + "-50_w.tif"
            f_name = "images/testinv/"+str(i) + "-50_f.tif"
            flow_name = "images/testinv/"+str(i) + "_50_flow.npy"
