...
def train():
    ...
    train_dir_fixed = glob.glob(os.path.join(args.train_dir_fixed, '*tif'))
    train_dir_moving = glob.glob(os.path.join(args.train_dir_moving, '*.tif'))

    DS = Dataset(files_fixed=train_dir_fixed, files_moving=train_dir_moving)
    ...
    # Training loop.
    with torch.no_grad():
        for input_fixed, input_moving,name in DL:

            # [B, C, D, W, H]
            print(name[0])
            name_img = name[0].split(".")[0]
            ...
            # Save images
            m_name = "Result/temp/bspl_crop/"+ f"{name_img}_m.tif"
            m2f_name = "Result/temp/bspl_crop/" + f"{name_img}_w.tif"
            f_name = "Result/temp/bspl_crop/"+ f"{name_img}_f.tif"
            flow_name = "Result/temp/bspl_crop/"+ f"{name_img}_flow.npy"
            disp_name = "Result/temp/bspl_crop/" + f"{name_img}_disp.npy"
