...
from Model.model_bspl import U_Network, SpatialTransformer
import cv2
...
def train():
    ...
    start = 0
    # 读入fixed图像
    f_img = normalize(cv2.imread("images/20_round_A.tif",0)[start:start+height,start:start+width][np.newaxis,np.newaxis,:])
    f_img = torch.Tensor(f_img)
    input_fixed = f_img.to(device).float()

    C_img = normalize(cv2.imread("images/20_round_C.tif",0)[start:start+height,start:start+width][np.newaxis,np.newaxis,:])
    G_img = normalize(cv2.imread("images/20_round_G.tif",0)[start:start+height,start:start+width][np.newaxis,np.newaxis,:])
    T_img = normalize(cv2.imread("images/20_round_T.tif", 0)[start:start+height,start:start+width][np.newaxis,np.newaxis, :])
    ...
    UNet = U_Network(2, nf_enc, nf_dec,img_height=height,img_width=width).to(device) # len(vol_size )是数据维度。
    #device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    UNet.load_state_dict(torch.load(args.checkpoint_path, map_location=device))
    STN = SpatialTransformer((height,width)).to(device)
    ...
    with torch.no_grad():
        for i,img  in enumerate(imglist):
            ...
            m2f, flow, disp = UNet(input_moving, input_fixed)
            # Save model checkpoint
            #save_file_name = os.path.join(args.model_dir, '%d.pth' % i)
            #torch.save(UNet.state_dict(), save_file_name)
            # Save images
            m_name = "Result/temp/bspl/"+str(i) + "-bspl_m.tif"
            m2f_name = "Result/temp/bspl/"+str(i) + "-bspl_w.tif"
            f_name = "Result/temp/bspl/"+str(i) + "-bspl_f.tif"
            flow_name = "Result/temp/bspl/"+str(i) + "_bspl_flow.npy"
            disp_name = "Result/temp/bspl/" + str(i) + "_bspl_disp.npy"
