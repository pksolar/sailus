...
class U_Network(nn.Module):
    def __init__(self, dim, enc_nf, dec_nf, bn=None, full_size=True,img_height=1024,img_width=1024):
        """
        ...
        self.dec.append(self.conv_block(dim, dec_nf[2] + enc_nf[1], dec_nf[3], batchnorm=bn))  # 4
        self.dec.append(self.conv_block(dim, dec_nf[3], dec_nf[4], batchnorm=bn))  # 5

        if self.full_size:
            self.dec.append(self.conv_block(dim, dec_nf[4] + 16, dec_nf[5], batchnorm=bn))
        if self.vm2:
            self.vm2_conv = self.conv_block(dim, dec_nf[5]+2, dec_nf[6], batchnorm=bn)
        ...
        self.transform = transformation.CubicBSplineFFDTransform(ndim=3,img_size=(img_height,img_width), svf=True, cps=(15,15))
