...
class UNet(nn.Module):
    def __init__(self, in_channel=1, out_channel=4, training=False):
        ...
        self.out_conv =  nn.Conv3d(2, 4, 1, 1)
    ...
    def forward(self, x):
        ...
        out = F.relu(F.max_pool3d(self.encoder1(x), (2,2,2), (2,2,2)))# input,kernel ,stride
        t1 = out
        out = F.relu(F.max_pool3d(self.encoder2(out), (2,2,2), 2))
        t2 = out
        out = F.relu(F.max_pool3d(self.encoder3(out), (1,2,2),  (1,2,2)))
        t3 = out
        out = F.relu(F.max_pool3d(self.encoder4(out), (1,2,2),  (1,2,2)))
        # t4 = out
        ...
        out = F.relu(F.interpolate(self.decoder2(out), scale_factor=(1, 2, 2), mode='trilinear'))
        out = torch.add(out, t3)
        # output2 = self.map2(out)
        out = F.relu(F.interpolate(self.decoder3(out), scale_factor=(1, 2, 2), mode='trilinear'))
        ...
        output = torch.mean(out, dim=2, keepdim=True)
