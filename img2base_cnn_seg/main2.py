...
def main():
    ...
    all_files = glob.glob(
        r"E:\data\deep\image2base\image\img\*_A_img.tif")

    train_files = [f for f in all_files if "30_R" not in f]
    val_files = [f for f in all_files if "30_R"  in f]
    ...
    train_loader = data.DataLoader(train_dataset, batch_size=BATCHSIZE, num_workers=1, shuffle=False)
    ...
    if continuetrain == True:
        ...
        print(model_lists[-4])
        best_model = torch.load(model_lists[-4])['state_dict']
