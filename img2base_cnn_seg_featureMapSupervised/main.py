...
import cv2
def tensorshow(outputs,name="output"):
    batch_features = outputs[0].detach().cpu().numpy()
    normalized_features = (batch_features - np.min(batch_features)) / (
            np.max(batch_features) - np.min(batch_features))
    normalized_features = (normalized_features * 255).astype(np.uint8)
    for i, channel in enumerate(normalized_features):
        # 构建保存文件的路径和名称
        filename = f"debug/img/{name}_{i}.png"
        # 使用 OpenCV 保存图像
        cv2.imwrite(filename, channel)
...
def main():
    ...
    train_files = [f for f in all_files if "08h_"  in f ]#用08少批量数据进行训练。
    ...
    train_loader = data.DataLoader(train_dataset, batch_size=BATCHSIZE, num_workers=5, shuffle=True,pin_memory=True)
    ...
    for epoch in range(5000):
        ...
        # random.shuffle(train_files)
        start = time.time()
        for inputs,labels,featurelabel,msk,featuremsk,name in train_loader:
            # print("name:",name)
            idx += 1
            model.train()
            ...
            featurelabel = featurelabel.to(device,non_blocking=True)
            featuremsk = featuremsk.to(device,non_blocking=True)

            # 前向传播
            outputs = model(inputs) #b c h w
            tensorshow(inputs[:,4:8],"input")
            tensorshow(outputs,"output")
            tensorshow(featurelabel,"label")
            #
            # 计算损失
            # inputs_show = np.squeeze(np.squeeze(inputs.detach().cpu().numpy()))
            # for i in range(4):
            #     plt.imshow(inputs_show[i], cmap='gray')
            #     plt.savefig(f'debug/input_image_{i}.png')  # 将第i张图像保存到本地文件


            # outputs_show = np.squeeze(np.squeeze(outputs.detach().cpu().numpy()))
            # for i in range(4):
            #     plt.imshow(outputs_show[i], cmap='gray')
            #     plt.savefig(f'image_{i}.png')  # 将第i张图像保存到本地文件


            # lable_show =  np.squeeze(labels.detach().cpu().numpy())
            number = torch.sum(msk)
            loss_ce = criterion_CE(outputs*msk, labels*msk) *  2000/number # ce loss，github 上说还可以用 focal loss,weighted 等等
            loss_mse = 1-criterion_SSIM(outputs*featuremsk,featuremsk*featurelabel)
            # loss_mse = criterion_MSE(outputs*featuremsk,featuremsk*featurelabel)
            loss = loss_ce + 0.01* loss_mse
            # loss_feature =
            #记录loss
            ...
            optimizer.step()
            #打印loss
            print('Iter{} of {} loss {:.6f},loss_ce {:.6f},loss_mse {:.6f}'.format(idx,len(train_loader),loss.detach().cpu().numpy().item(),
                                                                                   loss_ce.detach().cpu().numpy().item(),
                                                                                   loss_mse.detach().cpu().numpy().item()))
        end = time.time()
        ...
        if epoch % 2 == 0:
            ...
            with torch.no_grad():
                for inputs,labels,msk in val_loader:  #
                    ...
                    msk = msk.to(device)
                    outputs = model(inputs) # b c h w  c 一般是4，b是batchsize，我需要输出它们。

                    tensorshow(inputs[:,4:8], "input")
                    tensorshow(outputs, "output")
                    tensorshow(featurelabel, "label")

                    loss_ce = criterion_CE(outputs * msk, labels * msk)  # ce loss，github 上说还可以用 focal loss,weighted 等等
                    # loss_mse = criterion_MSE(outputs * featuremsk, featuremsk * featurelabel)
                    loss_val =  loss_ce
                    ...
                    print('loss {:.6f},loss_ce {:.6f},loss_mse {:.6f}'.format(             loss.detach().cpu().numpy().item(),
                                                                                           loss_ce.detach().cpu().numpy().item(),
                                                                                           loss_mse.detach().cpu().numpy().item()))
