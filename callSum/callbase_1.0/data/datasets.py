...
def cropimage(img_lumin_list,img_label,l=256):
    ...
    for img in img_lumin_list:
        cropimg_list.append(img[start_y:start_y+l,start_x:start_x+l])
    ...
    def __getitem__(self, step):
        ...
        if self.norm == True:
            ...
            for i in range(len(img_list)):
                ...
                imgcrop_list[i] = imgcrop_list[i][np.newaxis, :]
        #归一化，并切块的图
        #增加一个维度：
...
class Dataset_epoch2(data.Dataset):
    def __init__(self,names,norm=True):
        #输入只读带有A的
        super(Dataset_epoch2,self).__init__()
        self.names = names
        self.norm = norm
        #所有的数据都读到了index_pair里
        self.index_pair = names
