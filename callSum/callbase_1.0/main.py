...
from data.datasets import Dataset_epoch , Dataset_epoch2
...
def main():
    ...
    total_dir = glob.glob("E:\code\python_PK\callbase\datasets/21/Res/Lane01/deepLearnData/*/intensity/*_A.npy")

    train_dir = total_dir[0:int(0.8*len(total_dir))] #

    train_dataset = Dataset_epoch(train_dir)
    train_loader = data.DataLoader(train_dataset,batch_size=BATCHSIZE,shuffle=True)
    val_dir = total_dir[int(0.8 * len(total_dir)):int(0.8 * len(total_dir))+8] #
    ...
    for epoch in range(500):
        ...
        with torch.no_grad():
            for inputs,labels in val_loader: #
                ...
                for i in range(c.shape[0]): #labels:  b c h w
                    label = label_[i] #label can only be 0-4
        ...
        # 打印这个每个碱基的预测：
        for i in range(5):
            print(dict_class[i],": ",accurate_show_list[i].avg.item())
