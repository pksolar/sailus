...
class Dataset_epoch12(data.Dataset):
    def __init__(self,names,norm=False):
        #输入只读带有A的
        super(Dataset_epoch12,self).__init__()
        self.names = names
        self.norm = norm
    ...
    def __getitem__(self, step):
        ...
        #剪裁img，msk，label
        imgcrop_list,labelcrop,maskcrop = cropimage(img_list,label1,mask,512) #最后数字是剪裁大小
        maskcrop = abs(maskcrop) #1,256,256
...
class Dataset_epoch(data.Dataset):
    ...
    def __getitem__(self, step):
        ...
        label1_path = self.index_pair[step].replace("intensity_norm","label").replace("A","label")
        mask_path_dir =r"E:\code\python_PK\callbase\datasets\08\Res\Lane01\deepLearnData\\"
        mask_path = mask_path_dir + img_pathA.split("\\")[-1].replace("A","mask")
        #print("mask_:",mask_path)
