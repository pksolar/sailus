...
def cropimage_test(img_lumin_list, img_label,  mask, l=256):
    """
    随机从图片中取 变成为l的正方形
    :param img: 亮度矩阵
    :return:
    ...
    return cropimg_list,img_label,mask
def Q99(img):
    """
...
class Dataset_epoch12_val(data.Dataset):
    ...
    def __getitem__(self, step):
        ...
        #剪裁img，msk，label
        imgcrop_list,labelcrop,maskcrop = cropimage_val(img_list,label1,mask,256) #最后数字是剪裁大小
        ...
        img[5:9] = np.multiply(img[5:9],labelcrop2)
...
class Dataset_epoch(data.Dataset):
    ...
    def __getitem__(self, step):
        ...
        label_tensor = label1_tensor.permute(2,0,1).contiguous()
...
class Dataset_epoch_test(data.Dataset):
    ...
    def __getitem__(self, step):
        ...
        # 剪裁img，msk，label
        imgcrop_list, labelcrop, maskcrop = cropimage_test(img_list, label1, mask)
        #maskcrop[maskcrop == -1] = 0  # 1,256,256
