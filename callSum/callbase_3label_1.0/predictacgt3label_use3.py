                        ...
                        for j in range(preds_np1.shape[2]):
                            ...
                            if msk_np[i, j] == 1:
                                if idx == 1:  # 说明是第一个cycle，创建包含几个reads的列表 ，
                                    ...
                                    listacgt2[listacgt_idx + len_point] = listacgt2[listacgt_idx + len_point] + dictacgt[pred0]+dictacgt[pred1]+dictacgt[pred2]  #
                                    # list_p_total[listacgt_idx+len_point].append(outputs_np_softmax[0,:,i,j].tolist())
                                    listacgt_idx = listacgt_idx + 1
                                else:  # 第二个cycle以后依次写入：
                                    listacgt2[listacgt_idx + len_point] = listacgt2[listacgt_idx + len_point] + dictacgt[pred0] #
                                    # list_p_total[listacgt_idx+len_point].append(outputs_np_softmax[0,:,i,j].tolist())
                                    listacgt_idx = listacgt_idx + 1
                    if idx == 99:
                        break
