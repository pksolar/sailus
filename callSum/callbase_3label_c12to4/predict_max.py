...
def main():
    ...
    #best_model = torch.load("savedir_pth/acc99.510510.pth.tar")['state_dict']
        ...
        for r in range(row):
            ...
            for c in range(col):
                ...
                for inputs, labels, msk in test_loader:  #
                    ...
                    print("input_shape:", inputs.shape[3])
                    ...
                    _, preds0 = torch.max(inputs[:, 0:4, :, :], 1)  #
                    _, preds1 = torch.max(inputs[:, 4:8, :, :], 1)  #
                    _, preds2 = torch.max(inputs[:, 8:12, :, :], 1)  #
                        ...
                        for j in range(preds_np1.shape[2]):
                            ...
                            if msk_np[i, j] == 1:
                                if idx == 1:  # 说明是第一个cycle，创建包含几个reads的列表 ，
                                    listacgt2.append(dictacgt[pred0])
                                    listacgt2[listacgt_idx] =listacgt2[listacgt_idx]+ dictacgt[pred1]+dictacgt[pred2]
                                elif idx ==2:
                                    continue

                                    # listacgt[-1] = dictacgt[preds[i][j]]
                                elif idx == 99:  # 说明是第一个cycle，创建包含几个reads的列表 ，
                                    listacgt2[listacgt_idx + len_point] = listacgt2[listacgt_idx + len_point] + dictacgt[pred2]  #
