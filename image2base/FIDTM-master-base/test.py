...
def validate(Pre_data, model, args):
    ...
    for i, (fname, img, fidt_map, kpoint) in enumerate(test_loader):
        ...
        with torch.no_grad():
            ...
            args['visual'] = True
            if args['visual'] == True:
                if not os.path.exists(args['save_path'] + '_box2/'):
                    os.makedirs(args['save_path'] + '_box2/')
                ori_img, box_img = generate_bounding_boxes(pred_kpoint, fname)
                point_map = generate_point_map(ori_img, pred_kpoint, f_loc, rate=1)
                ...
                res = np.hstack((ori_img,point_map))
                cv2.imwrite(args['save_path'] + '_box2/' + fname[0], point_map)
...
def generate_point_map(img_ori,kpoint, f_loc, rate=1):
    ...
    for i in range(0, len(pred_coor[0])):
        ...
        cv2.circle(point_map, (w, h), 1, (0, 255, 0), -1)
...
def generate_bounding_boxes(kpoint, fname):
    ...
    Img_data = cv2.imread(
        r'E:\code\python_PK\image2base\FIDTM-master-base\data\noname\test_data/images/' + fname[0])
