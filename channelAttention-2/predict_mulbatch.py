...
def main():
    ...
    best_model = torch.load("savedir_pth/acc99.299.pth.tar")['state_dict']
    ...
    with torch.no_grad():
        for inputs, labels in val_loader: #
            ...
            pred_np_array = pred.cpu().numpy().astype(int).squeeze()  # 1,1000,100
            for jj in range(pred_np_array.shape[0]):

                pred_np = pred_np_array[jj]
                str_acgt = ""
