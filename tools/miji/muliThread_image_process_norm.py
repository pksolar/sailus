...
def cal99_1(img):
    ...
    return p1,p99
def norm_mul(total_dir,base):
    for path in total_dir:
        path = path.replace("_A",base)
        img = np.load(path)
        p1, p99 = cal99_1(img)
...
# 所有cycle里的亮度矩阵路径
total_dir = glob.glob(r"E:\data\resize_test\08_resize_ori\res_deep_intent\Lane01\deepLearnData\*\intensity/*_A.npy")
# threading.Thread(target=,args=)


threads = []
# create threads for each channel
threadA = threading.Thread(target=norm_mul, args=(total_dir,"_A"))
threadC = threading.Thread(target=norm_mul, args=(total_dir,"_C"))
threadG = threading.Thread(target=norm_mul, args=(total_dir,"_G"))
threadT = threading.Thread(target=norm_mul, args=(total_dir,"_T"))
