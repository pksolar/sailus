...
def k_min(matrix,peak_sub,size = 11):
    ...
    # plt.show()
# coordDirTotal = r"E:\code\python_PK\callbase\datasets\08\Res\Lane01\sfile\\"
...
filename = "R002C106_A##R002C106_C.txt"
# coordDirTotal = r"C:\Users\Administrator\Documents\WeChat Files\wxid_izeosircs0gj22\FileStorage\File\2023-05\\\\"
...
for coordFile in glob.glob(os.path.join(coordDirTotal,filename)):#读取坐标。
    ...
    if '_'  in os.path.basename(coordFile):
        ...
        centerarr = zerosarrar[int(height / 2) - 500:int(height / 2) + 500, int(width / 2) - 500:int(width / 2) + 500]
        step  =4
        cols = int(1000/step)
        rows = int(1000/step)
        list_count = []
        for i in range(rows):
            for j in range(cols):
                count = np.sum(np.sum(centerarr[i*step:(i+1)*step,j*step:(j+1)*step]))
                list_count.append(count)
        count_arr = np.array(list_count)
        print("std:",np.std(count_arr))
        print("mean:",np.mean(count_arr))
        print("bianyiL:",np.std(count_arr)/np.mean(count_arr))
        print("ddd")