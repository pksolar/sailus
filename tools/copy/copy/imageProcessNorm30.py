...
import glob
# 所有cycle里的亮度矩阵路径
total_dir = glob.glob(r"E:\code\python_PK\bleeding\img_process\1.9.39_data\res_all_for_deep\deep/*/intensity/*.npy")
...
for i,path in enumerate(total_dir):
    num = int(path.split("\\")[-3][-3:])
    if num>=48 :
        print(path)
        print("here")
        img = np.load(path)
