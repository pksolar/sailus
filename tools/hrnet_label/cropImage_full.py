...
for (path,path_label) in zip(paths,path_labels) :
    ...
    os.makedirs(f"{val}/imgdata_full",exist_ok=True)
    os.makedirs(f"{val}/label_full", exist_ok=True)
    os.makedirs(f"{val}/mask_full", exist_ok=True)
    # os.makedirs("img2base/lab", exist_ok=True)
    np.save(f"{val}/imgdata_full/{cycname}.npy", img_npy)
    np.save(f"{val}/label_full/{cycname}.npy", label_npy)
    #np.save(f"{val}/mask/{idx:04d}.npy", msk)
    idx = idx + 1
    print("idx：", idx)
