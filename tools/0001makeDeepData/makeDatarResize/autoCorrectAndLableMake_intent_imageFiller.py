...
def basecall(step1_command_filter):
    #cmd = rf''
    print("thread_start")
    os.system(step1_command_filter[0])
    print("tread_done")
...
rootdir = r"E:\data\resize_test\52.2h_resize_ori\\" # 下一级目录是FOV
fovlsit = ["R001C001"]
save_root_dir = r"E:\data\testAuto\\"
for fov in fovlsit:
    print(fov)
    machineName = fov
    resdir = rootdir+"res_deep_intent"
    imageFilterDir = rootdir+"res_deep_imageFilter"
    ori_mappingdir = resdir

    step1_command_filter = rf"E:\software\DeepLearningDataMake\001SalusCall_imageFilter.exe {rootdir} {imageFilterDir} 1 100 1 1 -m"
    list_commend = list[step1_command_filter]
    thread_basecall = threading.Thread(target=os.system, args=(step1_command_filter,))
    thread_basecall.start()

    #
    # 步骤1，运行baseCall，为了生成用来mapping的文件fastq以及生成配准后的图像。
    step1_command = rf'E:\software\DeepLearningDataMake\002SalusCall_intent.exe {rootdir} {resdir} 1 100 1 1 -m'
