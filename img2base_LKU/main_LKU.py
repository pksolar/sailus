...
def main():
    ...
    train_files = [f for f in all_files if "08h"  in f ]
    ...
    train_loader = data.DataLoader(train_dataset, batch_size=BATCHSIZE, num_workers=4, shuffle=True,pin_memory=True)
    ...
    for epoch in range(5000):
        ...
        for inputs,labels,msk,name in train_loader:
            ...
            loss = criterion(outputs*msk, labels*msk)*2000 / number # ce loss，github 上说还可以用 focal loss,weighted 等等
            #记录loss
        ...
        if epoch % 2 == 0:
            ...
            with torch.no_grad():
                for inputs, labels, msk in val_loader:  #
                    ...
                    outputs = model(inputs)
                    number = torch.sum(msk)
                    loss_val = criterion(outputs * msk, labels * msk)*2000/number
