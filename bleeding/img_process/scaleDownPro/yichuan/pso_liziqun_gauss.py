...
def convolve(img_path,kernel, path_save, str_,imgnameA):
    ...
    dst = cv2.filter2D(dst, -1, kernel).astype(np.uint8)
...
def evaluate_kernel(params):
    ...
    with open('kernel_mapping_pso_gauss.txt', 'a') as f:
        np.savetxt(f, kernel)
    #对图像采用多线程卷积操作：
    threads = []
    ...
    for pathA in paths_A:
        ...
        path_save = r"E:\code\python_PK\bleeding\img_process\scaleDownPro\yichuan_pso_gauss\image\Lane01\{}".format(cycname)
    ...
    rootdir = r"E:\code\python_PK\bleeding\img_process\scaleDownPro\yichuan_pso_gauss\image\\"
    resdir = "res"
