...
def evaluate_kernel(params):
    ...
    with open('kernel_mapping.txt', 'a') as f:
        np.savetxt(f, kernel)
    #对图像采用多线程卷积操作：
    threads = []
    ...
    for pathA in paths_A:
        ...
        path_save = r"E:\code\python_PK\bleeding\img_process\scaleDownPro\yichuan\image\Lane01\{}".format(cycname)
    ...
    rootdir = r"E:\code\python_PK\bleeding\img_process\scaleDownPro\yichuan\image\\"
    resdir = "res"
    ...
    score = mappingrate + mappedreads / 10000
    print("score:",score)
...
ga = GA(func=evaluate_kernel, n_dim=3, size_pop=30, prob_mut=0.001,max_iter=10, lb=[-1, -1, 1], ub=[0, 0, 6], constraint_ueq=[constraint_func], precision=0.01)
