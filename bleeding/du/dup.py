...
def dup(odir: str, sample_id: str, fqgz: str, n_num: int = 0):
    ...
    # get sequence lines
    seq_f_temp = f"{odir}/{sample_id}_fastq.fq"
    cmd = f"zcat {fqgz} | perl -lane 'if ( $. % 4 == 2 ) {{ print \"$_\"; }}' > {seq_f_temp}"
    os.system(cmd)
    ...
    if int(n_num) == 0:
        ...
        seq_f = f"{seq_f_temp}.removeN"
        with open(seq_f_temp) as fh, open(seq_f, "w") as outf:
            for line in fh:
                seq = line.strip()
                if len(seq.split("N")) - 1 < int(n_num):
                    outf.write(f"{line}")
