...
class Dataset_3cycle(data.Dataset):
    ...
    def __getitem__(self, step):
        ...
        label_path = path.replace("img_ori", "label").replace("_" + basetype, ".npy")  # basetype: A.tif
        # name_file = name_file + " label:" + os.path.basename(label_path)
        label = np.load(label_path)
        # np.save("debug/label_" + name_file, label)
        # 这个msk必须时已经经过膨胀处理的mask。
        msk_path = path.replace(os.path.basename(path), f"{machine}_{fov}_msk.npy").replace("img_ori", "msk")
