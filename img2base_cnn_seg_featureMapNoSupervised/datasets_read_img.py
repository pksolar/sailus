...
class Dataset_3cycle(data.Dataset):
    ...
    def __getitem__(self, step):
        ...
        # 找到第一个出现的 "label" 子字符串的索引
        featurelabel_path = label_path.replace("label","featurelabel")
        featurelabel = np.load(featurelabel_path).astype(np.float32)
        ...
        featuremsk_path = msk_path.replace("msk","featuremsk")
        featuremsk = np.load(featuremsk_path)
        ...
        featurelabel = featurelabel[:, crop_size_x: crop_size_x + width, crop_size_y:crop_size_y + height].copy()
        featuremsk = featuremsk[crop_size_x: crop_size_x + width, crop_size_y:crop_size_y + height].copy()
        ...
        featuremsk_tensor = torch.from_numpy(featuremsk).unsqueeze(0).float()




        return img_tensor,label_tensor,featurelabel,msk_tensor,featuremsk_tensor,name_file
...
class Dataset_3cycle_test(data.Dataset):
    ...
    def __getitem__(self, step):
        ...
        print("cyc_m_number:",cyc_m_number)
        # front的时候：
        cyc_f_number = cyc_m_number - 1
        img_front_path = path.replace(cycname, 'Cyc' + str(cyc_f_number).zfill(3))
