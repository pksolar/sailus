...
import fastQ
import time
from datasets_read_img_onlyCELoss_resize_filter import Dataset_3cycle_test_Bigcrop_3label as Dataset_3cycle_test
...
def main():
        ...
        for inputs, labels, msk in test_loader:  #注意此处的msk 是否包含mapping上的
            ...
            for i in range(rows):
                for j in range(cols):
                    ...
                    mask_crop = torch.squeeze(msk_crop).numpy().flatten().astype(int)  # msk b,1,b,w
    ...
    fastQ.writeFq(save_path + 'Lane01_fastq_label.fq', labelacgt, 'R001C001')
