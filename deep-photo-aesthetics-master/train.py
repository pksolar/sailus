...
from modelmy.resnet_FT import resnet_gap_features
...
def update_results(epoch, predictions, targets, loss, weights, all_attributes, loss_data_for_df, corr_data_for_df):
    ...
    for i in range(current_batch_size):
        for j, k in enumerate(all_attributes):
            loss_data_for_df[k].append(masked_loss[i, j].data.cpu())
        loss_data_for_df["total_loss"].append(total_loss_per_sample[i].data.cpu())
