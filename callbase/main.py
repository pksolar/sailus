...
def main():
    ...
    total_dir = glob.glob("datasets/21/Res/Lane01/deepLearnData/*/intensity/*_A.npy")

    train_dir = total_dir[0:int(0.01*len(total_dir))]

    train_dataset = Dataset_epoch(train_dir)
    train_loader = data.DataLoader(train_dataset,batch_size=BATCHSIZE,shuffle=True)
    val_dir = total_dir[int(0.7 * len(total_dir)):int(0.8 * len(total_dir))]
